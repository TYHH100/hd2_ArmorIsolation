param(
    ...
    [string]$PackagePath = (Join-Path (Split-Path -Parent $PSScriptRoot) 'dist\cm14-isolated')
)

$ErrorActionPreference = 'Stop'
$taskGame = [IO.Path]::GetFullPath($GamePath).TrimEnd('\')
$taskPackage = [IO.Path]::GetFullPath($PackagePath).TrimEnd('\')
$taskReceiptPath = Join-Path $taskGame 'bin\CM14Isolation.install.json'
$taskPrefix = '9ba626afa44a3aa3.patch_'
$taskOwner = 'hd2-mods-test-cm14-isolation-v1'
...
if ($Action -eq 'Status') {
    [PSCustomObject]@{
        ...
        Addon = Test-Path -LiteralPath (Join-Path $taskGame 'bin\CM14Isolation.addon64')
        Log = Join-Path $taskGame 'bin\CM14Isolation.log'
    } | Format-List
...
if ($Action -eq 'Uninstall') {
    ...
    Write-Output 'CM14 test removed. Original mod files were not changed. The diagnostic log was retained.'
    return
}
...
$taskAddon = Join-Path $taskPackage 'addon\CM14Isolation.addon64'
$taskIni = Join-Path $taskPackage 'addon\CM14Isolation.ini'
...
foreach ($taskFile in $taskManifest.output) {
    Assert-Hash (Resolve-ChildPath $taskPackage $taskFile.path) $taskFile.sha256
}

# An old-ID override would still affect other suits even with the private copy present.
...
foreach ($taskPatch in $taskActivePatches) {
    ...
    try {
        ...
        for ($taskIndex = 0; $taskIndex -lt $taskCount; ++$taskIndex) {
            ...
            if ($taskOldIds.Contains($taskKey)) { $taskConflicts.Add($taskPatch.Name + ' ' + $taskKey) }
        }
    } finally { $taskStream.Dispose() }
}
if ($taskConflicts.Count) {
    throw ('Disable the original CM14 mod and overlapping old-ID mods using their manager before this test: ' +
...
foreach ($taskFile in $taskManifest.output) {
    if ($taskFile.name -notmatch '^9ba626afa44a3aa3\.patch_0(\.stream|\.gpu_resources)?$') {
...
foreach ($taskName in @('CM14Isolation.addon64', 'CM14Isolation.ini')) {
...
Write-Output "Installed CM14 test as $taskPrefix$taskNumber and CM14Isolation.addon64. Restart the game to test."
