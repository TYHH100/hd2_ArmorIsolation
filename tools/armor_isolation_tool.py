...
def analyze_source(source: Path, game: Path, reader_tools: Path, kits: Path,
    ...
    with game_context(game, kits, runtime_release) as effective_kits:
        return _analyze_source(source, game, reader_tools, effective_kits, names)


def _analyze_source(source: Path, game: Path, reader_tools: Path, kits: Path, names: Path | None = None):
...
def runtime_release(directory: Path | None = None):
    ...
    if (release.get("schema") != "hd2-armor-runtime-release/1" or
            ...
            release.get("patch_transport") != "hd2-armor-patch-footer/1" or
...
def inspect_local_game(game: Path, output: Path):
    from local_game_data import inspect_game
    runtime, release = runtime_release()
    return inspect_game(game, output, runtime, ROOT / "docs/armor-isolation-live-kits.json",
                        release["expected_game_dll_sha256"])
...
def package_readme(manifest):
    ...
    if modular:
        ...
        installation = (f"在管理器中导入 `{mod_directory}/`，{prerequisites}，"
                        ...
                        "原版与隔离版不能作为两个相同身份的模组同时启用。手动安装时只复制所选方案及必需依赖的补丁三件套到游戏 data 顶层，按现有补丁连续编号。")
        scope = ("本版支持单补丁及受支持的旧版/V1 模组目录。模块化输出保留完整选项树，"
                 "同一组件集合共用私有映射，各主补丁携带相同的内嵌配置；改变配件选择后须完整退出并重启游戏。"
        ...
        installation = "通过模组管理器安装 `patch/` 中的三个文件，保持同一补丁编号；手动安装则将三件套使用同一空闲且连续编号放入游戏 data 顶层。不要覆盖已有补丁。"
...
本包已完成资源生成及通用插件配置校验，尚未单独通过游戏画面验收。工具没有部署补丁或插件；本机适配数据可导出到游戏根目录，生成本包不需要 C++ 编译环境。

目标：{', '.join(names)}。

- {resource_description}
- `runtime/ArmorIsolation.addon64`：所有模组共用的固定插件，只需安装一份。
- 运行时配置已内嵌到每个主补丁，不需要单独安装 JSON。
...
- `tools/validate_runtime_profile.exe`：通用配置校验器，支持主补丁、旧配置，以及 --patches 游戏 data 目录；--capture-game 模式可只读提取运行中游戏的数据。

## 安装与测试

1. 正常退出游戏。停用源模组及其他仍覆盖相同原资源的版本；不能让原版覆盖包与隔离包同时生效。
2. {installation}
3. 将 `runtime/` 内容按目录结构部署到游戏 `bin/`。需要已启用 add-on 的 ReShade 6.5.1 / API17。新版通用 DLL 只需安装一次；以后安装、切换或卸载模组只操作补丁，无需另外管理 JSON。迁移时停用确认属于旧实验版的 `CM14Isolation`、`B01Isolation` 或 `ArmorIsolation_<包ID>` 插件。
4. 启动后在军械库加载目标，查看 `bin/ArmorIsolation.log` 中包 ID 和各 Kit 的 `APPLIED`，切换装备触发重建；检查体甲两种体型、头盔、混搭以及原本共用资源的其他装备。

新版插件启动后在自身修改前自动导出 `游戏根目录/ArmorIsolation.local-game.json`，EXE 可在游戏退出后复用；DLL/EXE 变化时旧文件失效。关键原生特征或数据结构改变会停止自动适配。适配包附 `compatibility/kits.json`，绑定本次游戏文件与数据，不能跨版本混装。日志每次启动清空一次，导出文件成功后覆盖，不累积历史版本。
5. 预览通过后再检查舰桥和任务场景。`APPLIED` 仅代表配置发布，不能替代外观验收。

回退：完整退出游戏，通过原部署方式卸载本包补丁（迁移旧包时同时移除旧的对应 JSON），再启用原模组。其他隔离包仍在使用时保留通用 DLL/INI。不要删除不确定归属的文件或造成补丁编号断层；配置只在启动时读取，关闭 INI 开关不能在运行中恢复已发布的配置。
...
def _generate_package(source: Path, game: Path, reader_tools: Path, kits: Path,
                      targets: list[str], output_root: Path,
                      coexist_manifests: list[Path] | None = None, log=print):
    ...
    with staging_directory(output_root) as staging:
        ...
        if manifest.get("runtime_compatibility"):
            ...
            (package / "compatibility").mkdir()
            shutil.copy2(kits, package / "compatibility/kits.json")
            manifest["compatibility_snapshot"] = "compatibility/kits.json"
        ...
        for row in manifest["output"]:
            ...
            if not PATCH_NAME.fullmatch(path.name):
                continue
            record = embedded_profile.append_profile(path, profile)
            record["path"] = path.relative_to(package).as_posix()
        ...
        if not patch_paths:
            raise ValueError("没有可嵌入配置的主补丁。")
        validation = staging / "validation-inputs"
        validation.mkdir()
        validation_paths = list(patch_paths)
        for path in coexist_manifests or []:
            document = json.loads(Path(path).read_text(encoding="utf-8"))
            existing = runtime_profile.make_runtime_profile(document, kits)
            existing_path = validation / f"{existing['package_id']}.json"
            runtime_profile.write_profile(existing, existing_path)
            validation_paths.append(existing_path)
        run_logged([str(runtime_dir / "validate_runtime_profile.exe"), "--inputs", *map(str, validation_paths)],
        ...
        manifest["addon"] = {"name": addon.name, "config_section": "ArmorIsolation",
                             ...
                             "profile_storage": embedded_profile.TRANSPORT, "embedded_profiles": embedded,
...
def main():
    ...
    parser.add_argument("action", choices=("analyze", "build", "inspect-local"))
    ...
    try:
        if args.action == "inspect-local":
            path, report = inspect_local_game(args.game, args.output)
