...
工具提供图形界面：选择源模组、分析候选装备、明确指定替换目标，再生成内嵌运行时配置的补丁。游戏端通过统一的 ReShade 插件 `ArmorIsolation.addon64` 加载配置。
...
- **本机适配**：无需联网，游戏启动后导出适配数据到游戏根目录，EXE 自动校验并复用；关键原生布局仍匹配的更新可继续生成。
- **独立输出**：不修改源模组，不自动部署到游戏目录，不覆盖已有生成结果。

## 兼容范围

当前已验证基准为 `1.0.0.18930`，另支持原生特征与数据布局仍匹配的新版本自动适配，**不保证兼容任意后续游戏更新或所有外观模组**。新版单文件程序：`dist/portable-compatible/ArmorIsolation.exe`，支持两种已审定完整装配模板及受约束的外部布局变化，已只读验证 `1.0.0.19099`（[验证与待验收边界](docs/native-19099-validation.md)），详见[本机适配文件说明](docs/portable-exe.md#本机适配文件)。

| 项目 | 要求或范围 |
| --- | --- |
| 游戏 | 本机已安装 Helldivers 2；已验证基准或通过原生特征检查的新版本，适配数据绑定 DLL/EXE SHA-256 |
...
5. 完整退出游戏，按生成包内的 README 安装补丁，并一次性安装新版通用插件，再启动游戏验证外观。
...
源模组 + 显式选择的目标装备
  → Python 解析资源与选项依赖
  → 分配私有资源 ID、重打包补丁
  → 将配置嵌入主补丁，校验并输出补丁和配套插件
  → 用户退出游戏后安装
  → ReShade 插件读取配置、校验目标并等待资源
  → 将目标装备关联到私有资源
```

生成包的 `manifest.json` 记录资源映射、输出文件与哈希等信息；运行时配置直接保存在主补丁尾部，由插件自动读取，无需逐模组安装 JSON。旧版独立 JSON 仅作为兼容入口；详见[补丁内嵌配置](docs/embedded-patch-profile.md)。
